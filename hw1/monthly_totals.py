...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    monthly = data.groupby(["name", data["month"]])["precipitation"].sum().unstack()
    counts = data.groupby(["name", data["month"]])["precipitation"].count().unstack()
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    # vecteur de distance/matrice de distance condensée = triangle supérieur de la matrice de distance
    distanceVector = pdist(df, func)
    # Convertir le vecteur de distances en matrice carrée de distances
    distanceMatrix = squareform(distanceVector)
    # Créer un DataFrame à partir de la matrice de distances
    new_df = pd.DataFrame(distanceMatrix, index=df.index, columns=df.index)
    return new_df
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    groupByName = df.groupby('name').first()
    # TODO: faites pivoter le dataframe de sorte que vous ayez lat/lon comme colonnes et les noms comme index
    latAndLonByName = groupByName[['latitude', 'longitude']]
    new_df = compute_pairwise(latAndLonByName, geodesic)
...
def correlation(u, v) -> float:
    ...
    idx_u = ~pd.isna(u)
    idx_v = ~pd.isna(v)
    idx = idx_u & idx_v # Combine les indices pour éviter les NaN dans u ou v
    ...
    corr = ((u[idx] - uAvg) * (v[idx] - vAvg)).mean() / (uStd * vStd)
...
def compute_pairwise_correlation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dailyPrecipitationByCity = df.pivot(index='name', columns='date', values='precipitation')
    new_df = compute_pairwise(dailyPrecipitationByCity, correlation)
    return new_df
...
def compute_pairwise_correlation_pandas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    new_df = df.pivot_table(index='date', columns='name', values='precipitation').corr()
    return new_df
